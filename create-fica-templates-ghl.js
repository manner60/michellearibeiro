...
const emailTemplates = [
  {
    name: "FICA Email 1 - Quick Question",
...
{{user.name}}`,
  ...
  {
    name: "FICA Email 2 - The Credit Explained",
...
{{user.name}}`,
  ...
  {
    name: "FICA Email 4 - It's a Check",
...
{{user.name}}`,
  ...
  {
    name: "FICA Email 5 - Common Mistakes",
...
{{user.name}}`,
  ...
  {
    name: "FICA Email 6 - Timing Matters",
...
{{user.name}}`,
  ...
  {
    name: "FICA Email 7 - Service Rates",
...
async function createEmailTemplates() {
  console.log('Creating email templates in GHL...\n');
  const createdTemplates = [];
  
  for (const template of emailTemplates) {
    try {
      const response = await axios.post(
        `https://services.leadconnectorhq.com/locations/${LOCATION_ID}/templates`,
        {
          name: template.name,
          type: 'email',
      ...
      console.error(`❌ Failed to create ${template.name}:`, error.response?.data?.message || error.message);
    }
  }
...
async function main() {
  try {
    console.log('🚀 Creating FICA Email Templates in GHL...\n');
    
    const templates = await createEmailTemplates();
    
    console.log(`\n✅ Created ${templates.length} email templates`);
    console.log('\n📋 Next Steps:');
    console.log('1. Go to GHL: Automation > Workflows');
    console.log('2. Create a new workflow named "FICA Tip Credit Campaign"');
    console.log('3. Set trigger: Tag "fica-prospect" is added');
    console.log('4. Add email steps using the templates created above');
    console.log('5. Add wait steps between emails (delays as noted)');
    console.log('\n📧 Email Sequence:');
    
    let day = 0;
    templates.forEach((t, i) => {
      console.log(`   Day ${day}: ${t.name}`);
      day += t.delay || 0;
    });
