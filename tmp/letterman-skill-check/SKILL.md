...
## Publication Targeting Rules

- **Do not hardcode or trust embedded publication IDs.**
- **Always verify the target publication before any write action** (create, update, delete, SEO update, section edits, newsletter cue changes).
- Verify by one of these methods:
  1. User explicitly provides the publication/storage ID
  2. Local Michelle-specific notes/memory contain an approved ID
  3. Call `GET /newsletters-storage` and match by publication name
- If the target publication is ambiguous, ask before writing.
- For Michelle-specific IDs, store them in local notes/memory, not in this shared skill.

---

## Create Article - 3 Modes

### WARNING BEFORE CREATING ANY ARTICLE — VERIFY FIRST:
1. **Which publication exactly?**
2. **What is the verified storage ID?** (from user, local notes/memory, or `GET /newsletters-storage`)
3. **Blank or AI-generated?**
4. **Local or Niche?** (for SEO strategy)
5. **How many words?**
6. **Image URL or generate one?**
7. **Check the content bank?**
...
- **Location names** relevant to the target publication area
...
```json
{
  "publication": "Target Publication Name",
  "stories": [
    {
      "id": 1,
