...
(async()=>{
 const results=[];
 for (const u of updates){results.push(await req(`/newsletters/${newsletterId}/sections/${u.id}`,'PUT',u.body));}
 console.log(JSON.stringify(results.map(r=>({status:r.status,id:r.data&&r.data._id,imageUrl:r.data&&r.data.imageUrl,title:r.data&&r.data.title})),null,2));
})();
