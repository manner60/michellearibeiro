...
function main() {
  ...
  if (opts.mode === 'doctor') {
    ...
    console.log(`✓ Loader version: ${VERSION} (Certified Catalog Edition)\n`);
    ...
    // Check skills directory (check both common locations)
    const workspaceSkillsDir = path.join(OPENCLAW_DIR, 'workspace', 'skills');
    const directSkillsDir = path.join(OPENCLAW_DIR, 'skills');
    
    let foundSkillsDir = null;
    let skillCount = 0;
    
    if (fs.existsSync(workspaceSkillsDir)) {
      foundSkillsDir = workspaceSkillsDir;
      skillCount = fs.readdirSync(workspaceSkillsDir).filter(f => 
        fs.statSync(path.join(workspaceSkillsDir, f)).isDirectory()
      ).length;
    } else if (fs.existsSync(directSkillsDir)) {
      foundSkillsDir = directSkillsDir;
      skillCount = fs.readdirSync(directSkillsDir).filter(f => 
        fs.statSync(path.join(directSkillsDir, f)).isDirectory()
      ).length;
    }
    
    if (foundSkillsDir) {
      console.log(`✓ Skills directory: ${foundSkillsDir}`);
      console.log(`  ${skillCount} skill(s) installed\n`);
    } else {
      console.log(`✗ Skills directory: NOT found`);
      console.log(`  Checked: ${workspaceSkillsDir}`);
      console.log(`  Checked: ${directSkillsDir}\n`);
    }
